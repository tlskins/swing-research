...
def cut_swings(src_video_path, write_path, src_nm, pitch_frames):
    ...
    while frame_num < total_frames-1:
        ...
        person_box = detect_person(frame, model, class_names)
        if person_box is None or not boxes_intersect(person_box, swing_body_box):
            continue
    ...
    pitch_idx = 0
    curr_pitch = pitch_frames[pitch_idx]
    for frame_num in swing_frames[1:]:
        # finalize group
        if frame_num - body_group['end_frame'] > SWING_GROUP_LEEWAY:
            # print('finalizing', body_group)
            ...
            if group_frames > 1 and len(body_group['sound_frames']) > 0:
                ...
                contact_score = 100
                contact_frame = body_group['sound_frames'][0]
                for s_frame in body_group['sound_frames']:
                    ...
                    if score < contact_score:
                        contact_score = score
                        contact_frame = s_frame

                body_group['score'] = frames_score + 0.7 * contact_score
                body_group['contact_frame'] = contact_frame
        ...
        if curr_pitch == frame_num:
            body_group['sound_frames'].append(frame_num)

        # increment counters
        while curr_pitch != -1 and frame_num >= curr_pitch:
            pitch_idx += 1
            curr_pitch = pitch_frames[pitch_idx] if pitch_idx < len(
                pitch_frames) else -1
    print(swing_groups)
    ...
    while len(swing_groups) > 0:
        ...
        swing_groups[:] = [g for g in swing_groups if not abs(
            sel['contact_frame'] - g['contact_frame']) < SWING_GROUP_BUFFER]

    print('sorting swings by timestamp...')

    def sort_swing_frame(group):
        return group['contact_frame']
    ...
    for swing_group in final_swing_groups:
        video_stream.set(cv2.CAP_PROP_POS_FRAMES, swing_group['contact_frame'])
        _, contact_frame = video_stream.read()
        person_box = detect_person(contact_frame, model, class_names)

        # initialize for output writers
        st_frame_num = swing_group['contact_frame'] - PRE_STRIKE_FRAMES + 1
        end_frame_num = swing_group['contact_frame'] + POST_STRIKE_FRAMES
...
if __name__ == "__main__":
    ...
    pitch_frames = [2, 44, 87, 94, 95, 107, 122, 157, 165, 197, 231, 232, 248, 252, 285, 286, 287, 298, 325, 343, 350, 366, 431, 433, 434, 435, 438, 461, 482, 541, 544, 545, 546, 548, 612, 613, 614, 620, 640, 655, 666, 678, 680, 681, 683, 686, 688, 689, 690, 691, 697, 698, 699, 700, 705,
    ...
    outputs = cut_swings(srv_video_path, './out', src_file_nm, pitch_frames)
    print(outputs)
